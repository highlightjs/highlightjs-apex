...
const t=e.regex,a="[a-zA-Z][a-zA-Z_0-9]*",n="\\b"+a+"\\b",c="@"+a,s=/(?=\s*\()/,o=/(?=\()/,i=/\s+/,r=/\b(?<!\?)\.\b/,l=/\b\?\.\b/,p=[{
match:r,scope:"punctuation"},{match:l,scope:"operator"}],b={scope:"number",
...
relevance:10}],L=[{match:t.either(...S),scope:"punctuation",relevance:0
}],N=e.inherit(e.APOS_STRING_MODE,{contains:[{match:/\\'/,scope:"literal"}],
scope:"string",relevance:0}),C=e.COMMENT("//",/[$\n]/,{relevance:0
}),D=[e.COMMENT("/\\*","\\*/",{relevance:0,contains:[{begin:/\w+@/,relevance:0
...
scope:"string",contains:[e.BACKSLASH_ESCAPE],relevance:0}]}),C],w={
match:t.concat(/(?<=\w|\b|\s|\d)/,t.either(...R),/(?=\w|\b|\s|;|\d|$)/),
scope:"operator",relevance:0},I={match:/(?<!\.)\bId\b/,scope:"type",relevance:8
},_=[{begin:/\b(list|set|map)(?=\s*<)/,beginScope:"type",end:/\>+/,
...
match:[a,/\[\]/],scope:{1:"type",2:"punctuation"},relevance:0}],U=[{
match:t.concat(c,/\b(?!\s*\()/),scope:"meta"},{
begin:[t.concat(c,/\b/),/\s*/,/\(/],beginScope:{1:"meta",3:"punctuation"},
end:/\)/,endScope:"punctuation",contains:[{match:[n,/\s*=/],scope:{1:"variable",
2:"operator"},contains:[w]},N,b],keywords:{literal:h}}],M={
match:[/\b[a-zA-Z0-9\.]*Exception/,i,a],scope:{1:"title.class",3:"variable"},
relevance:0},f={match:[/(?<!\.)/,n,/\s*(?=\=[^\>])/],scope:{2:"variable"},
relevance:0},k={match:[/(?<=\=\s*\()/,a,"(?=\\)\\s*"+a+")"],scope:{2:"type"},
relevance:0},F={match:[/\bnew\s+/,a,s],scope:{2:"type"},relevance:0},P={
match:[t.concat(/\b/,((...e)=>t.concat("(?!",e.join("|"),")"))(...g),/\??(?<=\.|:)/),a,s],
scope:{2:"title.function.invoke"},relevance:0},Y={scope:"params",end:/\)/,
endScope:"punctuation",relevance:0,keywords:v,illegal:u,contains:[b,N,D,_,...O,{
match:/\,|\(/,scope:"punctuation"},I,{match:[/(?<=\(|\,)\s*/,a,/(?=\s)/],scope:{
2:"type"}},{match:[i,a,/\s*(?=[,)])/],scope:{2:"variable"}}]},W=[{
begin:[/\btrigger/,i,a,i,"on",i,a],beginScope:{1:"keyword",3:"title.class",
5:"operator",7:"type"},end:/(?=\{)/,returnEnd:!0,contains:[D,{begin:/\(/,
end:/\)/,contains:[{
...
relevance:1,keywords:{type:m,keyword:u},contains:[{match:[/(?<=\bclass)\s+/,a],
scope:{2:"title.class"}},{returnEnd:!0,endsWithParent:!0,
beginKeywords:"implements extends",end:/\{/,contains:[O,{match:[n,/(?=\.)/],
...
2:"title.function"},starts:Y,relevance:0},{
match:[/(?<!new)(?<!return)(?<=\<|\>|\w)\s+/,a,o],scope:{2:"title.function"},
relevance:0,starts:Y},{begin:[/\benum\s+/,a,/\s*\{/],beginScope:{2:"type",
3:"punctuation"},end:/\}/,endScope:"punctuation",relevance:0,contains:[D,T,{
match:t.concat(n),scope:"variable.constant"}]}],H={match:[/\bswitch\s+on\s+/,a],
scope:{1:"keyword",2:"variable"}},x=[{
match:[t.concat(/\b/,t.either(...E)),/\s+(?!\()/,t.optional(/as\s+(user|system)\b/)],
scope:{3:"keyword"}}],G={
...
literal:v.literal,built_in:g};return{name:"Apex",
...
contains:[U,f,k,_,D,W,x,p,M,{match:[/\bfor\b\s*/,/\(/,a,i,a,/\s*/,/:/],scope:{
3:"type",5:"variable",7:"operator"}},N,F,y,P,O,b,w,L,T,I,{
begin:[/\[/,/\s*(?=(SELECT|FIND)\b)/],end:/\]/,beginScope:{1:"punctuation"},
endScope:"punctuation",scope:"soql",relevance:10,endsWithParent:!0,keywords:G,
contains:[{begin:/\bSELECT\b/,beginScope:"keyword",end:/\bFROM\b/,returnEnd:!0,
keywords:G,contains:[T,L,{match:[/(?=[\s\,])/,a,/(?=[\s\,])/],scope:{2:"subst"}
}]},b,w,N,T,{match:/\(|\)/,scope:"punctuation"},p,{begin:[/\bFROM\b/,i],
...
match:[/(?<=:)/,/\s*/,n,/(?!\()/],scope:{3:"variable"}},{
match:[/(?<=:|\.)/,a,/(?=\s*\()/],scope:{2:"title.function.invoke"}},{match:/:/,
scope:"operator"}],illegal:"::"},H,p]}}})();export default hljsGrammar;