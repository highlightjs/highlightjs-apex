...
const t=e.regex,c="[a-zA-Z][a-zA-Z_0-9]*",a="[a-zA-Z_0-9]*",n="\\b"+c+"\\b",s="@"+c,o=/(?=\s*\()/,i=/\s+/,r=/\{/,l=/\s*\}/,p=/\(/,b=/\)/,u={
...
return t.concat("(?!",e.join("|"),")")}const A={scope:"number",
...
relevance:10}],F=[{
match:t.either(",",/;/,/(?<=\w)\>/,/\<(?=\w)/,/\{|\}/,/\(|\)/,/\{|\}/),
scope:"punctuation",relevance:0},_],P=e.inherit(e.APOS_STRING_MODE,{
scope:"string",relevance:0,contains:[{match:/\\'/,scope:"literal",relevance:0}]
}),Y=[{begin:"`",end:"`",scope:"string"},{begin:/'/,end:/'/,scope:"string"},{
begin:/"/,end:/"/,scope:"string"}],x=e.COMMENT("//",/[$\n]/,{relevance:0
}),W=/(\[\d+\])*/,H=/(?:@link|@see)\s+/,G=t.either("#",m),B=[{
match:[/\{*/,H,t.concat(c,m,c,m,c,W),/(?=[\}\s])/],scope:{2:"doctag",
3:"title.function"}},{match:[/(?<!\{\s*)@see\s+/,t.concat(c,G,c,W,/(?=\s)/)],
scope:{1:"doctag",2:"title.function"}},{match:[/(?<!\{\s*)@see\s+/,d],scope:{
1:"doctag",2:"link"}},{match:[/(?<!\{\s*)@see\s+/,t.concat(n,/(?=\s)/)],scope:{
1:"doctag",2:"title.class"}},{begin:/@example(?!\s+\*\s+\{)/,
beginScope:"doctag",end:/(@|\*\/)/,returnEnd:!0,starts:{endsWithParent:!0,
subLanguage:["apex","xml","javascript"]}}],K=[{
begin:[/\{*/,H,a,t.concat(G,c),/\s*\(/],beginScope:{2:"doctag",3:"title.class",
4:"title.function"},end:b,contains:[u,{match:[i,c,/\s*(?=[,)])/],scope:{
2:"variable"}},{match:[c,/(?=\s)/],scope:{1:"type"}},{match:/,/,
scope:"punctuation"},x,u]},{match:[/\s/,H,t.concat(a,G,c),/(?=[\}\s])/],scope:{
2:"doctag",3:"title.function"}},{match:[r,H,t.concat(c,G,c),l],scope:{
2:"doctag",3:"title.function"}},{match:[r,H,c,l],scope:{2:"doctag",
3:"title.class"}},{begin:[r,H,/(?=")/],beginScope:{2:"doctag"},end:l,contains:Y
},{begin:[r,H],beginScope:{2:"doctag"},end:l,returnEnd:!0,
subLanguage:["markdown","xml"],contains:[{match:d,scope:"link"}]
}],Q=[e.COMMENT("/\\*","\\*/",{relevance:0,contains:[{match:"<br>",relevance:0
},{begin:/\w+@/,relevance:0},{match:[/@(?:exception|throws)/,i,c],scope:{
1:"doctag",3:"title.class"},relevance:0},B,K,{scope:"doctag",begin:"@param",
end:/$/,contains:[{scope:"variable",variants:[{match:c}],endsParent:!0}]},{
variants:Y,contains:[e.BACKSLASH_ESCAPE],relevance:0},{begin:/[0-9]+\.[0-9]+/,
end:/\.*\s+/,scope:"number",excludeEnd:!0},{
begin:t.either(/\d{4}-\d{1,2}-\d{1,2}/,/\d{1,2}\/\d{1,2}\/\d{4}/),
scope:"literal"},{begin:[r,"@code"],beginScope:{2:"doctag"},starts:{end:l,
contains:[u,{begin:r,end:l,skip:!0}],subLanguage:["apex","xml","javascript"]}},{
begin:[r,s],beginScope:{2:"doctag"},end:l},{match:s,scope:"doctag",relevance:0
},u]}),x,u],z={match:t.either(...w),scope:"operator",relevance:0},V=[{
...
match:[c,/\[\]/],scope:{1:"type",2:"punctuation"},relevance:0}],Z=[{
match:t.concat(s,/\b(?!\s*\()/),scope:"meta"},{scope:"meta",
begin:[t.concat(s,/\b/),/\s*/,p],beginScope:{3:"punctuation"},end:b,
endScope:"punctuation",contains:[{match:[n,/\s*=/],scope:{1:"keyword",
2:"operator"},contains:[z]},P,A],keywords:{literal:D}}],j={
match:[/\b[a-zA-Z0-9\.]*Exception/,i,c],scope:{1:"type",3:"variable"},
relevance:0},X=[{match:[/(?<!\.)/,n,/\s*(?=\=[^\>])/],scope:{2:"variable"},
relevance:0},{match:[n,"(?=\\s+"+c+/\s*\=/+")"],scope:{1:"type"},relevance:0},{
match:[n,i,n,/\s*(?=\=[^\>])/],scope:{1:"type",3:"variable"},relevance:0}],q={
match:[/(?<=\=\s*\()/,c,"(?=\\)\\s*"+c+")"],scope:{2:"type"},relevance:0},$={
match:[/\bnew\s+/,c,o],scope:{2:"type"},relevance:0},J={begin:p,
beginScope:"punctuation",end:b,endScope:"punctuation",relevance:0,keywords:f,
contains:[]},ee=[{match:[/(?<=\.)/,c,o],scope:{2:"title.function.invoke"},
relevance:1,contains:[...g],starts:J},{match:[/^\s*/,v(...S),c,o],scope:{
3:"title.function.invoke"},relevance:1,starts:J}]
;J.contains=[P,$,Q,z,V,M,A,E,ee,{keywords:{KEYWORDS:f},
match:t.concat(/(?<=\s|\(|,)/,v(...D),c,/\b/,/(?!\.)/),scope:"variable",
relevance:0},{match:/\(|,|\./,scope:"punctuation",relevance:0},{match:c,
scope:"variable"}];const te={scope:"params",end:b,endScope:"punctuation",
relevance:1,keywords:f,contains:[A,P,Q,V,...M,{match:/,|\(/,scope:"punctuation"
},E,{match:[/(?<=\(|,)\s*/,c,/(?=\s)/],scope:{2:"type"}},{
match:[i,c,/\s*(?=[,)])/],scope:{2:"variable"}}]},ce={
match:[/\bswitch\s+on\s+/,c],scope:{1:"keyword",2:"variable"}};return{
...
contains:[Q,Z,P,A,q,V,[{begin:[/\btrigger/,i,c,i,"on",i,c],beginScope:{
...
scope:"punctuation"},M],relevance:0}]},{begin:[/\benum\s+/,c,/\s*\{/],
beginScope:{2:"type",3:"punctuation"},end:l,endScope:"punctuation",relevance:0,
contains:[Q,_,{match:t.concat(n),scope:"variable.constant"}]},{
match:[/(public|private|protected)\s+/,c,o],scope:{1:"keyword",
2:"title.function"},starts:te,relevance:1},{
match:[/(?<!new)(?<!return)(?<=\<|\>|\w)\s+/,c,o],scope:{2:"title.function"},
relevance:1,starts:te}],[{match:/as\s+(user|system)\b/,scope:"keyword",
relevance:5}],j,{match:[/\bfor\b\s*/,p,c,i,c,/\s*/,/:/],scope:{3:"type",
...
contains:[A,z,P,_,{match:/\(|\)/,scope:"punctuation",relevance:0},...g,{
begin:[/\bFROM\b/,i],beginScope:{1:"keyword"},end:/(?=\bWHERE\b|\]|\s|\))/,
contains:[{match:c,scope:"type"},...g]},{
...
relevance:0},{match:[/(?<=:|\.)/,c,o],scope:{2:"title.function.invoke"},
relevance:0},{match:/:/,scope:"operator",relevance:0}],illegal:"::"
},ce,X,z,F,g,{match:t.concat("(?<=this.)",c,"(?!\\()"),scope:"variable"},{
match:[t.concat(/\b/,v(...U)),c,/(?=\s*\{)/],scope:{2:"property"},relevance:0}]}
